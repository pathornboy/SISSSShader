...
#pragma exclude_renderers flash
	...
	inline fixed4 LightingTranslucent(SurfaceOutput s, fixed3 lightDir, fixed3 viewDir, fixed atten)
	{
		...
		fixed3 transLight =  (transDot)* s.Alpha * _SubColor.rgb;
		...
		float spec = pow(nh, s.Specular*100.0) * s.Gloss;
		fixed3 diffAlbedo = (s.Albedo * ((_LightColor0.rgb * max(0.7,s.Alpha)) + pow(s.Albedo.rgb*_SubColor.rgb * s.Alpha, _Power) )  * diff + ((_LightColor0.rgb * _SpecColor.rgb) )*spec) * (atten * 2);


		// Regular Phong
		//half3 r = normalize(2*(dot(s.Normal, lightDir)) - lightDir);
		//fixed diff = max(0, dot(s.Normal, lightDir));
		//float rv = max(0, dot(r, viewDir));
		//float spec = pow(rv, s.Specular*100.0) * s.Gloss;
